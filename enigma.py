...
#  the remaining 3 files are text files. The Enimga program# put in readme file
#  takes plain text and encypts the message.               # modify txt file but not the python files to run the code.(run engima.py )(python 3)
############################################################
 # enigma machine is built with rotos ect and input files and output files
import rotors
import reflector
import inputs
import plugboard
    ...
    def encrypt_text(self, inputFile, outputFile):
        ...
        while new_char:
            new_char = inputFile.read(1)
                #F remove: if len(new_char) == 1 and ord("A") <= ord(new_char) <= ord("Z"):
